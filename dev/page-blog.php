	...
	<?php 
		if($search){ ?>
			<section class="blogSearchTerm">
				<div class="container">
					Showing results for: <span>"<?php echo $search; ?>"</span>
				</div>
			</section>
		<?php
		}
			...
			<?php
				...
				if ( $query->have_posts() ) {
					while ( $query->have_posts() ) {
						...
						<article class="blogPageArticle" data-aos="fade-up">
							<a href="<?php the_permalink(); ?>">
								<div class="blogPageArticlePhoto" style="background-image: url('<?php the_post_thumbnail_url(); ?>  ');">
									<div class="content"></div>
								</div>
								<div class="blogPageArticleText">
									<div class="blogPageArticleTextDate">
										<?php 
											$postDate = get_the_date(); echo $postDate;
										?>
									</div>
